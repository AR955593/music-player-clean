...
class AudioController {
  ...
  void _setupAudioPlayer() {
    ...
    audioPlayer.positionStream.listen((_) {
      ...
      // isPlaying.notifyListeners(); // Invalid use of protected member
    });
  }

  Future<void> loadSongs() async {
    try {
      // Ensure the plugin has the necessary permission (some devices/emulators
      // require using the plugin's permission request in addition to permission_handler).
      final hasPerm = await audioQuery.permissionsStatus();
      if (!hasPerm) {
        debugPrint(
          'Audio permission not granted to OnAudioQuery; requesting...',
        );
      ...
      debugPrint('AudioController: fetched ${fetchedSongs.length} songs');
      ...
      debugPrint('AudioController.loadSongs error: $e\n$st');
    }
  }
